...
sleep 15
docker-compose exec neon-local psql -U neon -d neondb -c 'SELECT 1' || echo "Database not ready yet, continuing..."
...
docker-compose exec app npx drizzle-kit migrate --config=drizzle.config.js || echo "Migration failed, but continuing..."
